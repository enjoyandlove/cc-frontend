...
export class LocationExistsGuard implements CanActivate {
  ...
  hasLocation(id: number): Observable<boolean> {
    return this.store
      .select(fromStore.getLocationsById)
      .pipe(map((location: any) => !!location[id]), take(1));
  }

  checkStore(): Observable<boolean> {
    return this.store.select(fromStore.getLocationsById).pipe(
      tap((locations: any) => {
        const locationId = this.locationId;

        const hasSchedule = Object.keys(locations).length
          ? Boolean(locations[this.locationId].schedule.length)
          : false;
