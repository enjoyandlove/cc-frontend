...
export class JobsEffects {
  ...
  loadEmployers$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap(() => {
      return this.service.getEmployers().pipe(
  ...
  createEmployer$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap((employer) => {
      ...
      return this.employerService.createEmployer(employer, search).pipe(
        map((createdEmployer: IEmployer) => new fromActions.CreateEmployerSuccess(createdEmployer)),
  ...
  editEmployer$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap((employer) => {
      const search = new HttpParams().append('school_id', this.session.g.get('school').id);
      return this.employerService.editEmployer(employer.id, employer, search).pipe(
  ...
  deleteEmployer$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap((id) => {
      const search = new HttpParams().append('school_id', this.session.g.get('school').id);
      return this.employerService.deleteEmployer(id, search).pipe(
