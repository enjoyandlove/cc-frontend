      ...
      concatMap((action: fromActions.GetCaseStatusById) => {
        ...
        if (exclude_external_cases !== null) {
          params = params.append(
            'exclude_external_cases',
            exclude_external_cases ? 'true' : 'false'
          );
        }
        return this.casesService.getCaseStatusById(id, params).pipe(
          map((caseStatus: ICaseStatus) => new fromActions.GetCaseStatusByIdSuccess(caseStatus)),
          catchError((error) =>
            of(new fromActions.GetCaseStatusByIdSuccess(parseErrorResponse(error)))
          )
        );
      })
    )
